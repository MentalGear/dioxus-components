...
pub fn Chart(props: ChartProps) -> Element {
    ...
    // encodes visually, so the hidden table should carry it too. Radar and
    // RadialBar keep the generic single-series reduction -- "percent of
    // the total" isn't a meaningful reading of a radial bar's own value
    // the way it is for a pie slice.
...
mod tests {
    ...
    // -- Stage-2 polar kinds (Pie/Radar/RadialBar) ---------------------------
    //
    // Each kind renders the same `Harness` used by every Cartesian test
    // above (it's generic over `kind`). Radar/RadialBar are still `s2-
    // refactor`'s placeholder stub; Pie is this lane's (`s2-polar`) own
    // real `series::pie::render` -- see that module's own doc for why
    // `sample_config`'s two series (desktop/mobile) make this a *stacked*
    // (two-ring) pie, not the single-ring case (covered in detail by
    // `series::pie`'s own test module instead).
