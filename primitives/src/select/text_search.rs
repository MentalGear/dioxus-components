...
mod tests {
    ...
    #[test]
    fn test_code_to_char_all_arms() {
        // Parametrized over every one of `code_to_char`'s 36 `KeyCode -> char` arms so a
        // mutation to any single arm (wrong char, deleted arm, swapped return) is caught.
        let cases = [
            ("KeyA", 'a'),
            ("KeyB", 'b'),
            ("KeyC", 'c'),
        ...
        assert_eq!(cases.len(), 36, "expected all 36 code_to_char arms covered");

        for (code, expected) in cases {
            assert_eq!(
                code_to_char(code),
                Some(expected),
                "code_to_char({code:?}) should map to {expected:?}"
            );
        }

        // Unmapped codes fall through to `None`.
        assert_eq!(code_to_char("Unidentified"), None);
