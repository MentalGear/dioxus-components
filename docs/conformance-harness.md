...
**Findings recorded but not fixed** (structural or out-of-round-scope; filed as fresh backlog rows — see `docs/backlog.md` rows 36-38):
...
- **axe (static rules), 2026-09-03 (`docs/backlog.md` row 34):** implemented across 33 of 45 specs (72 scan-call sites, up from 3 specs / 4 calls) — see "axe (static rules)" above for the full account, the shared `playwright/axe.ts` helper, and this round's findings.
...
| `aria-selected` | 13 |
| `role=` | 11 |
| axe scans | **3 specs of 32** (superseded 2026-09-03: 33 of 45, 72 scan-call sites — see "axe (static rules)" above) |
| `aria-hidden` | **0** |
| `overflow` / `scrollY` | **0** |
