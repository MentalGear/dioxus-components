...
test.describe('Axe automated scan', () => {
  test('loaded (unchecked) has no automatically detectable a11y issues', async ({ page }) => {
    ...
    await expectNoAxeViolations(page, 'checkbox: unchecked', { exclude: [CONTRAST_TRACKED_ELSEWHERE] });
  });
  ...
  test('checked has no automatically detectable a11y issues', async ({ page }) => {
    ...
    await expectNoAxeViolations(page, 'checkbox: checked', { exclude: [CONTRAST_TRACKED_ELSEWHERE] });
  });
});
