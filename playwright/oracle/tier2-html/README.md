...
`main-thread.spec.ts` (dev-docs/backlog.md row 45, added 2026-09-18) is this tier's broadest rule so far by normative source, not by "HTML forms": it cites the W3C Long Tasks API (`PerformanceLongTaskTiming`, the 50ms long-task definition) and the W3C Event Timing API (`durationThreshold`, `interactionId`, `processingStart`/`processingEnd`) rather than the WHATWG forms chapters above — the same "cite a section of a standard, not an opinion" policy this tier's header states, applied to a different corner of the platform, the way `native-dialog.spec.ts`'s `<dialog>`/UI-Events citations and `touch-focus-zoom.spec.ts`'s citations already sit in this tier without being about forms either. It asserts no `longtask` over 50ms and no `event` entry over 200ms during every overlay's open/close and every form control's primary interaction (dev-docs/recommended-implementations.md §11's reference thresholds), using a shared helper (`../../main-thread.ts`) and a best-of-3-attempts retry policy documented in the spec file's own header ("Robustness under load") to stay trustworthy on a sandbox that builds concurrently. Chromium-only, per that same header and row 45's own caveat.
