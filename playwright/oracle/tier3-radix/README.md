...
Per conformance-harness.md: `scroll-lock.spec.ts` implemented (6 rules, landed 2026-08-30); `rtl.spec.ts` implemented (dev-docs/backlog.md row 13, landed 2026-09-19) — RTL support across Tabs/RadioGroup/ToggleGroup/Toolbar/Menubar/Navbar (roving-focus arrow-key flip, cited to `@radix-ui/react-roving-focus`), DropdownMenu.Sub/ContextMenu.Sub (open/close arrow key + side, cited to `@radix-ui/react-menu`), Slider (pointer/keyboard value mapping, cited to `@radix-ui/react-slider`), Select/ScrollArea (`dir` attribute presence only, no Radix-cited keyboard change), and Resizable/Calendar (extrapolated, no Radix/shadcn-with-Radix-primitive original exists for either — labelled as such in the file, not cited); `chart.spec.ts` written red-first (dev-docs/research/chart-2026-09-19.md, the Chart round, 2026-09-19) — Chart has no Radix primitive at all (§2.4 of that research doc), so this one cites Recharts' `accessibilityLayer` @ `86ad3632ff3f83a742001ae6fc079dd27961a2a4` for its keyboard rule (R5) and the WAI-ARIA Graphics Module 1.0 for its role rules (R1 `img`, R3 `graphics-symbol`) instead of a Radix demo — the folder name is kept per that research doc's own §6.5 option (b), not renamed to `tier3-opinion/`; otherwise not implemented — `onCloseAutoFocus`, `aria-hidden` on background content, and collision-aware repositioning have no standing tier-3 oracle of their own (focus restore ended up calibrated against the vendored APG reference instead, in tier 1, once that upgrade landed).
