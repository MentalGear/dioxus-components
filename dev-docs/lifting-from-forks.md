...
**ISC-licensed sources (2026-09-19, Chart round finding):** `d3-shape`/`d3-array` (Mike Bostock, ISC) are outside this repo's original MIT/Apache/BSD framing but sit in the same permissive family for these purposes — ISC's terms (use/copy/modify/distribute with the copyright+permission notice retained) impose no obligation this repo's own "concept port" category above doesn't already satisfy: an algorithm PORT (JS to Rust, necessarily a rewrite, not a text copy) needs no provenance header, but citing the source file+SHA+licence in the commit message and the ported module's own doc comment is the standing practice (`primitives/src/chart/engine/scale.rs`/`curve.rs` are the first instance). Re-verify the specific licence before treating any other ISC/MIT/Apache/BSD-family source the same way — this is a category, not a blanket clearance.

**File-level copyleft sources, e.g. MPL-2.0 (2026-09-19, Chart round finding):** `feral-dot-io/leptos-chartistry` (a sibling Leptos chart library, read for the Chart round) is MPL-2.0 — weak copyleft, but *file-level*, not simply "OK with attribution" the way MIT/Apache/BSD/ISC are for this repo's own "concept port" doctrine above. Copying an MPL-2.0 file's actual content into a crate declared `MIT OR Apache-2.0` (this repo's own terms) would put that content under MPL-2.0 obligations (source availability for that file, licence notice) the crate's own declared terms don't carry. Rule: read for the algorithm/design shape, re-derive independently, cite the source in the commit message — never copy-paste, even with attribution. (Stricter than Shape C "port the concept" above reads for a permissive source: for MPL-2.0 and other file-level-copyleft licences, re-derivation is mandatory, not just cheaper-than-lifting.) Concretely: `dev-docs/research/chart-forks-2026-09-19.md` §4.1 found that `leptos-chartistry`'s own from-memory-shaped Rust transcription of the same Steffen (1990) monotone-cubic paper `d3-shape` (ISC) implements silently drops a clamp term the canonical source has — a second, independent argument for re-deriving against the canonical (here, ISC) source directly rather than a sibling library's own port, licence question aside.
...
| 2026-08-31 | Phase 4.2 native `<dialog>` modality: `use_dialog_open_driver`/`use_dialog_close_sync` (`lib.rs`); `DialogContent` split into `DialogContentModal`/`DialogContentNonModal` real child components (mirroring `popover.rs`'s `PopoverModalContent`/`PopoverNonModalContent`); `use_dialog_backdrop_dismiss` (`dialog.rs`, new); `AlertDialogContent` cfg-split directly (always modal, no `is_modal` branch). Web arm renders a real `<dialog>` (`role="alertdialog"` kept explicit for `AlertDialog`), `open` never bound as an attribute; native/Blitz arm and `is_modal: false` `Dialog` byte-for-byte unchanged. `Sheet`'s `.dx-sheet` CSS gained `max-width/max-height: none` (Chromium's `dialog:modal` UA cap would otherwise shrink a full-bleed side sheet) | C, concept + construction only — no fork code copied; the eval-channel shape (`use_dialog_open_driver`/`use_dialog_close_sync`) is a from-scratch re-derivation of `dev-docs/phase4-spike-findings.md`'s spike hooks of the same name (never-merged branch `spike/native-dialog`), and the component-boundary split is this repo's own `popover.rs` pattern, not the fork's | `dev-docs/phase4-spike-findings.md` (spike branch, local only, never pushed) | — | New `playwright/oracle/tier2-html/native-dialog.spec.ts`: rule 1 (clipping escape) red → green, native-reference CALIBRATION green throughout; rule 2 (background inertness: blocked click + blocked `.focus()`) red → green; rule 3 (Escape + signal sync, 5 reopen cycles) red → green; rule 4 (focus restore, control) green throughout; rule 5 (nested dialogs: close order + focus return) red → green. `dialog.spec.ts`/`alert-dialog.spec.ts`/`sheet.spec.ts` Tab-cycle assertions updated for the native trap's documented `<body>` intermediate stop (harness correction, not a behavior change); all three, plus `oracle-focus-restore.spec.ts` (Dialog CONTROL) and `oracle/tier3-radix/scroll-lock.spec.ts`, stayed/went green |
| 2026-09-18 | `SidebarProvider`'s inline `style` string written on one line instead of a multi-line raw string (cosmetic SSR-output fix, so the rendered attribute no longer line-breaks) | A, cherry-pick `-x`, author preserved, no path conflicts | `DioxusLabs/components` (upstream) PR #288, `sumitpsm` | `49e3f17` | None — cosmetic output fix, nothing behavioral to test; `backlog.md` row 66 |
| 2026-09-18 | Tooltip close-fade CSS: `dx-tooltip-fade-out` keyframes, `opacity: 0`/`pointer-events: none` replacing `display: none` on the closed state so `animation` can apply | A + adaptation — literal `0.2s` replaced with the `--dx-motion-duration-slow` token (`check-css-literals.sh`); added a `prefers-reduced-motion` override matching `input_otp`/`spinner`'s convention | `Wervice`, fetched via upstream PR #293's head (`refs/pull/293/head`) — no longer present on any branch of that fork itself, per row 66's fork re-check | `a704c517` (landed as `6a5130b`) | `playwright/assert-fade-out.ts` (new shared sampler) + `tooltip.spec.ts`'s/`hover-card.spec.ts`'s/`popover.spec.ts`'s close-fade regression cases; `backlog.md` rows 7 and 19 (19's `use_popover_shown_while_mounted` call-site swap is what makes this CSS observable on the web arm at all — landed same lane, original construction, no source to cite) |
| 2026-09-18 | Typeahead for `DropdownMenu`/`ContextMenu`/`Menubar`: `find_next_match` prefix-over-timed-buffer matcher + `TypeaheadState`, wired into each host's keydown handler and `menu_sub.rs`'s Sub content handlers; additive `text_value` channel through `collection.rs` | C, concept only — rewritten against this crate's own `collection.rs`/`select/context.rs` buffer-and-timeout idiom (`dioxus_sdk_time::sleep`), no code lifted | `dignifiedquire/dx-components` `typeahead.rs` (78 ln), read for shape only — explicitly **not** `select/`'s own Levenshtein matcher (`recommended-implementations.md` §8) | — | 12 new rows in `playwright/oracle/tier1-apg/keyboard-matrix.spec.ts` (menu sections); `backlog.md` row 11 |
| 2026-09-18 | `pub mod portal` API surface (module doc + rustdoc + 2 SSR tests); `Toggle`'s unstyled `class` prop (merge-not-clobber in the themed wrapper, same fix shape in `toggle_group`'s wrapper); public `CalendarDayState`/`use_calendar_day_state`/`calendar_day_attributes` (upstream `DioxusLabs/components#199`) | C, three concept ports — none of the three fork diffs actually taken | Each flagged by a different fork during the fork-mining survey (`adopt-fork-fixes-results.md` row 12, entries 269–271): `matchish@df16e449` (portal), `catan2001@a7a6a9f1` (toggle class, whose own default of an unprefixed `"toggle"` class was rejected), `jaysonmaw@3c43ce75` (calendar day state, whose own version was flagged as panicking on out-of-context misuse — reason enough on its own not to lift it) | — | `portal.rs`'s 2 new `#[cfg(test)]` SSR tests; `toggle.rs`/`toggle_group` component tests (6 new); `calendar.rs`'s day-state/attribute tests (12 new, 38 total in the module); `backlog.md` row 12 |
| 2026-09-18 | Table, Data Table, Drawer, Navigation Menu, Resizable — five `component-backlog.md` catalog components | original composition against house infrastructure (`Dialog`; `Combobox`'s own `selectable`/`listbox`/`collection` layer; `slider.rs`'s pointer-drag math via `pointer.rs`/`move_interaction.rs`; the top-layer/anchor-positioning engine); no fork code lifted for any of the five | `rust-ui/dioxus-ui` and `dignifiedquire/dx-components` read for shape/architecture only per each component's own row in `component-backlog.md` (the Table/Data Table split, Navigation Menu's viewport trick, Drawer's popper/focus_scope entanglement, Resizable's whole-repo-rewrite scope) — none adoptable, all confirmed still reference-only at landing time | — | `oracle/tier1-apg/{sortable-table,disclosure-navigation,window-splitter}.spec.ts` (new); `table.spec.ts`, `data_table.spec.ts`, `drawer.spec.ts`, `navigation_menu.spec.ts`, `resizable.spec.ts`; `component-backlog.md`'s five rows |
