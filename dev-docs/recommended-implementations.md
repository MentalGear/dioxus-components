...
> **Update (2026-09-04) — an anchor *width* contract, closing the styled `Select` full-viewport-width report.** Position was the only thing this file's collision-avoidance work ever corrected; nothing published a trigger's *width* to a themed stylesheet, so `select/style.css`'s `min-width: 100%` (correct pre-migration, wrong once its listbox's containing block became the viewport) had no engine-level fix to fall back on when its own `@supports (anchor-name: --a) { min-width: anchor-size(width); }` rule turned out dead under `#[css_module]`'s `@supports`-scoping gap (`dev-docs/backlog.md` rows 32/47). Fixed with the same "measure, then publish a fact" shape this section's flip/shift work already uses: `use_anchor_position_fallback`'s `reposition()` now writes the trigger's live width to a `--dx-anchor-width` custom property on the content element, unconditionally (every engine, every call, kept fresh by the existing scroll/resize/`visualViewport` tracking) — a themed stylesheet opts in with `min-width: var(--dx-anchor-width, 100%)`. The shared `@supports` stylesheet also declares `--dx-anchor-width: anchor-size(width)` on the same marker classes, verified by execution to substitute correctly through a custom property in this sandbox's Chromium, as a CSS-native belt alongside the JS suspenders. See `primitives/src/top_layer.rs`'s `use_anchor_position_fallback` doc, "Anchor width contract," for the full mechanism.
